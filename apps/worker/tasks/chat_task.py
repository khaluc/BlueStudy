    ...
    with sessions.begin() as db:
        ...
        if image_requested and image_doc is None:
            ...
            if turn.status=='failed':
                ...
                trace[index].update(status='failed',detail='Chưa hoàn tất. Bạn có thể gửi lại yêu cầu.')
            ...
            prompt=('Bạn là BlueStudy, trợ lý học tập cho học sinh lớp 9 Việt Nam. '
                'Trò chuyện bằng tiếng Việt dễ hiểu, hỗ trợ học tiếng Anh, gợi ý từng bước và hỏi để kiểm tra hiểu bài. '
            ...
            try:
                ...
                if image_doc:
                    ...
                    answer,name=vision(storage.read(image_doc.source_key),turn.message,context['recent_history'])
                    metadata={'provider':'ollama','input_kind':'image','fallback_reason':None}
                else:
                    answer,name=model.ask(message)
                    metadata=getattr(model,'last_metadata',{})
                turn.answer=answer
                turn.provenance={'model':name,**metadata,'review_status':'unreviewed'}
